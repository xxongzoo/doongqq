...
//하늘색
  function init() {
    const MODEL_PATH = 'https://doongebucket.s3.ap-northeast-2.amazonaws.com/sh_doong_animation.glb';
    const canvas = document.querySelector('#c');
    const backgroundColor = 0xc8b0d0;

    // Init the scene
    scene = new THREE.Scene();
    scene.background = new THREE.Color(backgroundColor);
    scene.fog = new THREE.FogExp2(0xc8b0d0, 0.018);
    ...
    loader.load(
      ...
      function (gltf) {
        ...
        model.traverse(o => {
          ...
          if (o.isBone) {
            ...
            if (o.name === 'mixamorigSpine2R') waist = o;
          }
          if (o.isMesh) {
            o.castShadow = true;
          ...
          if (o.isBone && o.name === 'mixamorigSpine2R') { 
            waist = o;
          }
        });

        model.scale.set(5, 5, 5);
        model.position.y = -16.5; //높이
        scene.add(model);
      ...
      // Set initial position for the model 모델의 높이
      model.position.y = -10;
    },
    undefined,
    function (error) {
  ...
  const floorMaterial = new THREE.MeshPhongMaterial({ color: 0xae99b5, shininess: 0 });
